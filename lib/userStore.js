...
export async function issueCert(programId, subjectId, subjectName, programName, userName) {
  ...
  if (isGlobalAccount()) {
    const sb = getSupabase(); const prof = getProfile()
    if (sb && prof?.supabaseId) {
      // Await the insert so the cert is in DB before the PDF is generated
      // and the QR can be scanned on any device.
      try {
        await sb.from('certificates').insert({
          user_id:      prof.supabaseId,
          id:           cert.id,
        ...
        // Insert failed (e.g. duplicate) — cert is still in localStorage
      }
    }
  }
  return cert
