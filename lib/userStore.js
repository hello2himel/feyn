...
if (CERT_ID_SUFFIX_LENGTH % 2 !== 0) {
  throw new Error('CERT_ID_SUFFIX_LENGTH must be even')
}
...
function sanitizeEmail(value) {
  return sanitizeText(value, INPUT_LIMITS.email).toLowerCase()
}

function sanitizeUsername(value) {
  const cleaned = sanitizeText(value, INPUT_LIMITS.username).toLowerCase()
  return cleaned.replace(/[^a-z0-9._-]/g, '')
}

function sanitizeProfileInput(data = {}) {
  return {
    name: sanitizeText(data.name, INPUT_LIMITS.name),
    username: sanitizeUsername(data.username),
    email: sanitizeEmail(data.email),
  }
}

function withTimeout(promise, ms = PULL_TIMEOUT_MS) {
  return Promise.race([
    promise,
...
async function queryWithTimeout(queryPromise, ms = PULL_TIMEOUT_MS) {
  try {
    return await withTimeout(queryPromise, ms)
  } catch (e) {
    if (e?.message === 'timeout')
      return { data: null, error: { code: 'TIMEOUT', message: 'Query timed out' } }
    return { data: null, error: { code: 'QUERY_FAILED', message: e?.message || 'Query failed' } }
  }
}

function sanitizeProfilePatch(data = {}) {
  const out = {}
  if (Object.prototype.hasOwnProperty.call(data, 'name'))
    out.name = sanitizeText(data.name, INPUT_LIMITS.name)
  if (Object.prototype.hasOwnProperty.call(data, 'username'))
    out.username = sanitizeUsername(data.username)
  if (Object.prototype.hasOwnProperty.call(data, 'email'))
    out.email = sanitizeEmail(data.email)
  return out
}
...
function makeCertId() {
  ...
  if (cryptoObj?.randomUUID) {
    return `FEYN-${cryptoObj.randomUUID().replace(/-/g, '').slice(0, CERT_ID_SUFFIX_LENGTH).toUpperCase()}`
  }
  if (cryptoObj?.getRandomValues) {
    const bytes = new Uint8Array(CERT_ID_SUFFIX_LENGTH / 2)
    cryptoObj.getRandomValues(bytes)
    const randomHex = Array.from(bytes).map(b => b.toString(16).padStart(2, '0')).join('').toUpperCase()
    return `FEYN-${randomHex}`
  }
  throw new Error('Secure random source unavailable for certificate ID generation')
}

function getProfileCache() { return _profileCache }
function setProfileCache(profile) { _profileCache = profile || null }
function clearProfileCache() { _profileCache = null }
function getCertsCache() { return Array.isArray(_certsCache) ? _certsCache : [] }
function setCertsCache(certs) { _certsCache = Array.isArray(certs) ? certs : [] }
function clearCertsCache() { _certsCache = [] }
function setPendingProfileCache(profile) { _pendingProfile = profile || null }
function getPendingProfileCache() { return _pendingProfile }
function clearPendingProfileCache() { _pendingProfile = null }
function setPendingEmailCache(email) { _pendingEmail = email || null }
function getPendingEmailCache() { return _pendingEmail }
function clearPendingEmailCache() { _pendingEmail = null }
...
export function attachAuthListener() {
  ...
  sb.auth.onAuthStateChange(async (event, session) => {
    if (event === 'INITIAL_SESSION' || event === 'SIGNED_IN') {
      ...
      clearProfileCache()
      clearCertsCache()
      clearPendingProfileCache()
      clearPendingEmailCache()
...
function fg(queryPromise) {
  ;(async () => {
    try {
      const result = await withTimeout(queryPromise, WRITE_TIMEOUT_MS)
      if (result?.error) {
...
export function getProfile()  { return getProfileCache() }
export function isSignedIn()  { return !!getProfileCache() }
...
export async function signUpGlobal({ name, username, email, password }) {
  try {
    ...
    const clean = sanitizeProfileInput({ name, username, email })

    if (clean.username) {
      const { data: usernameTaken, error: usernameError } = await sb
        .rpc('is_username_taken', { candidate_username: clean.username })
      if (usernameError) {
        return { ok: false, error: 'Unable to verify username availability. Please try again.', field: 'username' }
      }
      if (usernameTaken) return { ok: false, error: 'That username is already taken.', field: 'username' }
    }

    const { data, error } = await sb.auth.signUp({
      email:    clean.email,
      password: password,
      options:  { data: { name: clean.name, username: clean.username } },
    })
    ...
    setPendingProfileCache({
      name: clean.name,
      username: clean.username,
      email: clean.email,
    })
    return { ok: true, needsOtp: true, email: clean.email }
  ...
  try {
    ...
    if (error) {
      ...
      if (msg.includes('email not confirmed') || msg.includes('not confirmed')) {
        ...
        if (!otpRes.error) {
          setPendingEmailCache(cleanEmail)
          return { ok: true, needsOtp: true, email: cleanEmail }
        }
...
export async function verifyOtp({ email, token, type = 'email' }) {
  try {
    ...
    if (!sb) return { ok: false, error: 'Cloud sync is not available right now.' }
    const cleanEmail = sanitizeEmail(email)
    const cleanToken = sanitizeText(token, INPUT_LIMITS.otpToken).replace(/\s+/g, '')

    const { data, error } = await sb.auth.verifyOtp({
      email: cleanEmail, token: cleanToken, type,
    })
...
export async function resendOtp(email) {
  try {
    ...
    const { error } = await sb.auth.signInWithOtp({
      email: cleanEmail, options: { shouldCreateUser: false },
    })
...
async function _finaliseSignIn(user, session, sb) {
  ...
  const pending = getPendingProfileCache()

  const { data: dbProfile } = await sb
    .from('profiles').select('*').eq('id', userId).maybeSingle()

  const profile = {
    name:       sanitizeText(pending?.name || dbProfile?.name || user.user_metadata?.name || 'User', INPUT_LIMITS.name),
    username:   sanitizeUsername(pending?.username || dbProfile?.username || user.user_metadata?.username || ''),
...
async function _pullFromSupabase(userId, sb) {
  ...
  const [
    ...
    queryWithTimeout(sb.from('lesson_progress').select('lesson_key,watched_at,q_idx,answers,saved_at').eq('user_id', userId)),
    queryWithTimeout(sb.from('enrollments').select('subject_key').eq('user_id', userId)),
    queryWithTimeout(sb.from('certificates').select('*').eq('user_id', userId)),
    queryWithTimeout(sb.from('watch_positions').select('lesson_key,pct,pos_seconds,saved_at').eq('user_id', userId)),
    queryWithTimeout(sb.from('user_preferences').select('key,value').eq('user_id', userId)),
  ])
  ...
  if (!e3)
    setCertsCache((certRows || []).map(r => ({ id: r.id, programId: r.program_id, subjectId: r.subject_id, programName: r.program_name, subjectName: r.subject_name, userName: r.user_name, issuedAt: r.issued_at ? new Date(r.issued_at).getTime() : Date.now() })))
...
export async function saveProfile(data) {
  ...
  const merged = { ...getProfile(), ...sanitizeProfilePatch(data), updatedAt: Date.now() }
...
export function getSubjectProgress(programId, subjectId, subject) {
  ...
  const p = getProgress()
  // Cards carry `lessonPaths` (flat [{topic, lesson}]) instead of the
  // full topic tree — see attachSubjectStats in data/courseHelpers.js.
  const all = subject?.topics?.length
    ...
    : (subject?.lessonPaths || []).map(x => `${programId}/${subjectId}/${x.topic}/${x.lesson}`)
...
// ── Certificates ──────────────────────────────────────────────────────
export function getCerts() { return getCertsCache() }
...
/**
 * Issues (or re-issues) a certificate. Idempotent: the same course
 * always yields the same cert id for the same user.
 *
 * `attribution` carries the denormalized publisher and mentor names.
 * They are stored on the row on purpose — a certificate must stay
 * truthful after the course is edited, the publisher renames itself,
 * or the crediting mentor leaves. Reading them live at verification
 * time would silently rewrite history.
 */
export async function issueCert(programId, subjectId, subjectName, programName, userName, attribution = {}) {
  ...
  const cert = existing || (() => {
    const c = {
      id:          makeCertId(),
      programId,   subjectId,
      subjectName: sanitizeText(subjectName, 160),
      programName: sanitizeText(programName, 160),
      userName:    sanitizeText(userName, INPUT_LIMITS.name) || 'Student',
      publisherName: sanitizeText(attribution.publisherName || '', 160) || null,
      mentorNames:   sanitizeText(attribution.mentorNames || '', 300) || null,
      issuedAt: Date.now(),
    }
    setCertsCache([...certs, c])
    return c
  })()
  ...
  const uid = await getLiveUid()
  if (!uid) {
    console.error('[Feyn] issueCert: no active session')
    return { cert, dbOk: false, dbError: 'no session' }
  }
