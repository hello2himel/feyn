...
export function attachAuthListener() {
  ...
  sb.auth.onAuthStateChange(async (event, session) => {
    if (event === 'INITIAL_SESSION' || event === 'SIGNED_IN') {
      ...
      // The supabase.js onAuthStateChange listener and this one fire in
      // registration order — not guaranteed. Setting it here explicitly
      // ensures _authFetch uses the real JWT when the queries below fire.
...
async function _finaliseSignIn(user, session, sb) {
  ...
  // Set token immediately — session is live at this point.
  // _seedPromise may not have resolved yet if this is called very
  // early (e.g. signUp with email confirm disabled returns session
  // synchronously). Setting it here guarantees all DB calls below
  // use the real JWT, not the publishable key fallback.
