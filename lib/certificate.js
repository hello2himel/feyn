...
const SITE_URL = process.env.NEXT_PUBLIC_SITE_URL || 'https://feyn.netlify.app'
...
export async function downloadCertificate({
  ...
  subjectDesc, coachSignatureUrl,
}) {
  // Normalise cert fields — handles both camelCase (from issueCert) and
  // snake_case (from DB pull via older localStorage cache).
  const certNorm = {
    id:          cert.id,
    userName:    cert.userName    || cert.user_name    || 'Student',
    subjectName: cert.subjectName || cert.subject_name || '',
    programName: cert.programName || cert.program_name || '',
    issuedAt:    cert.issuedAt    || (cert.issued_at ? new Date(cert.issued_at).getTime() : Date.now()),
  }
  ...
  // QR points to the verify page — DB is the single source of truth.
  // Netlify rewrites /verify/:certId → /verify/?id=FEYN-xxx
  const qrUrl = `${SITE_URL}/verify/${certNorm.id}`
  ...
  doc.setFontSize(9)
  doc.setTextColor(...P.gold)
  doc.text(certNorm.id, CR, HDR_Y + 4, { align: 'right' })
  ...
  const name  = cert.userName || cert.user_name || 'Student'
  ...
  doc.setFontSize(14)
  doc.setTextColor(...P.gold)
  doc.text((cert.subjectName || cert.subject_name || '').toUpperCase(), cx, courseY, { align: 'center' })
  ...
  doc.setFontSize(7.5)
  doc.setTextColor(...P.text3)
  const issuerStr = (cert.programName || cert.program_name)
    ? `${cert.programName || cert.program_name}  ·  Feyn Learning Platform`
  ...
  const dateStr = new Date(cert.issuedAt || cert.issued_at).toLocaleDateString('en-US', {
  ...
  doc.save(`Feyn-Certificate-${slug(cert.subjectName || cert.subject_name)}-${slug(cert.userName || cert.user_name)}.pdf`)
}