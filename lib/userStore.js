...
//
// Flow:
//   sign up  → signUpGlobal()  → sendOtp()  → verifyOtp() → done
//   sign in  → signInGlobal()  → sendOtp()  → verifyOtp() → done
...
// ── Global sign-up — step 1: create account + send OTP ────────────────
...
export async function signUpGlobal({ name, username, email, password }) {
  try {
    ...
    // Store pending profile — finalised after OTP verify
    const pending = {
    ...
    // If Supabase immediately returned a session (email confirm disabled), we're done
    if (data.session) {
    ...
    if (otpRes.error) {
      // OTP send failed — account was created but we can't verify. User can sign in later.
      return { ok: true, needsOtp: false, otpFailed: true }
    }
...
// ── Global sign-in — step 1: verify password + send OTP ───────────────
  ...
  try {
    ...
    if (error) {
      ...
      if (msg.includes('invalid') || msg.includes('credentials') ||
          msg.includes('wrong') || msg.includes('email not confirmed') ||
          msg.includes('not confirmed')) {
        // Could be unconfirmed — send OTP as the verification step
        const otpRes = await sb.auth.signInWithOtp({
          email: email.trim(),
        ...
        if (!otpRes.error) {
          ...
          return { ok: true, needsOtp: true, email: email.trim(), reason: 'confirm' }
        }
    ...
    // Signed in successfully via password — no OTP needed
...
// ── OTP verify — step 2: enters the 6-digit code ─────────────────────
...
// ── Internal: finalise sign-in after auth ─────────────────────────────
async function _finaliseSignIn(user, session, sb) {
  ...
  // Check for pending profile from sign-up flow
...
export async function upgradeToGlobal({ email, password, username } = {}) {
  try {
    ...
    // After OTP verify, syncLocalToSupabase is called via _finaliseSignIn
    // We store a flag to trigger it
    lsSet('ff_upgrade_pending', true)

    return { ok: true, needsOtp: !otpRes.error, email: email.trim() }
  } catch (e) {
    return { ok: false, error: e?.message || 'Something went wrong. Please try again.' }
  }
}
...
async function syncLocalToSupabase(userId, sb) {
  ...
  if (rows.length) await sb.from('lesson_progress').upsert(rows, { onConflict: 'user_id,lesson_key' }).catch(() => {})
  if (enrolled.length) {
...
export function enroll(programId, subjectId) {
  ...
  if (!list.includes(key)) {
    ...
    if (isGlobalAccount()) {
      ...
      if (sb && p?.supabaseId) {
        sb.from('enrollments').upsert({ user_id: p.supabaseId, subject_key: key }, { onConflict: 'user_id,subject_key' }).catch(() => {})
      }
    }
  }
}
...
export function setLastVisited(programId, subjectId, topicId, lessonId) {
  ...
  const all = getWatchPositions()
  all[key] = { ...(all[key] || {}), savedAt: Date.now() }
  lsSet(WATCH_POS_KEY, all)
}
