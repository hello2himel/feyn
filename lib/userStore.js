...
function makeCertId() {
  ...
  if (cryptoObj?.randomUUID) {
    return `FEYN-${cryptoObj.randomUUID().replace(/-/g, '').slice(0, 16).toUpperCase()}`
  }
  ...
  return `FEYN-${Date.now().toString(36).toUpperCase()}-${Math.random().toString(36).slice(2, 8).toUpperCase()}`
}
  ...
  try {
    ...
    if (error) {
      ...
      if (msg.includes('email not confirmed') || msg.includes('not confirmed')) {
        ...
        if (!otpRes.error) {
          lsSet('ff_pending_email', cleanEmail)
...
export async function verifyOtp({ email, token, type = 'email' }) {
  try {
    ...
    if (!sb) return { ok: false, error: 'Cloud sync is not available right now.' }
    const cleanEmail = sanitizeEmail(email)
    const cleanToken = sanitizeText(token, 24).replace(/\s+/g, '')
...
async function _pullFromSupabase(userId, sb) {
  ...
  if (!e3)
    lsSet(KEYS.CERTS, (certRows || []).map(r => ({ id: r.id, programId: r.program_id, subjectId: r.subject_id, programName: r.program_name, subjectName: r.subject_name, userName: r.user_name, issuedAt: r.issued_at ? new Date(r.issued_at).getTime() : Date.now() })))
