...
export async function loadPermissions(sb, userId) {
  ...
  // subjects — and library resources — it is credited on.
  let mentorSubjectIds = []
  let mentorLibraryResourceIds = []
  if (mentor?.id) {
    const [subjRes, resRes] = await Promise.all([
      sb.from('subject_mentors').select('subject_id').eq('mentor_id', mentor.id),
      sb.from('library_resource_mentors').select('resource_id').eq('mentor_id', mentor.id),
    ])
    mentorSubjectIds = (subjRes.data || []).map(r => r.subject_id)
    mentorLibraryResourceIds = (resRes.data || []).map(r => r.resource_id)
  }
...
/**
 * Library-resource editability. Same ladder as canEditSubject:
 *   admin/editor  → any resource under that publisher
 *   mentor        → only resources they are credited on
 */
export function canEditLibraryResource(perms, resource) {
  if (!perms || !resource) return false
  if (perms.isAppAdmin) return true
  const role = perms.roleByPublisher[resource.publisher_id]
  if (role === 'admin' || role === 'editor') return true
  if (role === 'mentor') return perms.mentorLibraryResourceIds.includes(resource.id)
  return false
}

/** May upload a new library resource under this publisher (editor and up). */
export function canCreateLibraryResource(perms, publisherId) {
  return hasPublisherRole(perms, publisherId, 'editor')
}
