...
import { coaches, getCoach } from '../../data/courseHelpers'
...
export default function CoachPage({ coach, subjects }) {
  const totalLessons = subjects.reduce((acc, { subject }) =>
    acc + subject.topics.reduce((a, t) => a + (t.skills||[]).reduce((b,s)=>b+(s.lessons||[]).length,0), 0), 0)
                ...
                {subject.topics.map(topic => (
                  <div className="coach-topic" key={topic.id}>
                    ...
                    <div className="coach-lessons">
                      {(topic.skills||[]).flatMap(s=>s.lessons||[]).map((lesson, i) => (
