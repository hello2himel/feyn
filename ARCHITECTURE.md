...
**Files are never read from Storage directly — there is no SELECT policy on the bucket at all.** Every read, public or not, goes through [`pages/api/library/file/[resourceId].js`](pages/api/library/file/[resourceId].js): it checks `status`/`can_edit_library_resource_id()` with the caller's own token, then hands back a 60-second signed URL from the service-role client. An `<iframe src="/api/library/file/{id}">` follows that redirect and renders a PDF or image inline; anything else falls back to open/download buttons. **HTML is the exception:** Supabase Storage forces `text/html` objects to `text/plain` (anti-phishing), so a signed-URL redirect would show the page's source. For `kind='html'` the route reads the object server-side and serves it itself as `text/html`, with `Content-Security-Policy: sandbox allow-scripts …` (deliberately *without* `allow-same-origin`) so uploaded HTML runs in an opaque origin and can't touch the app's session. Consequence: uploaded pages can't use `localStorage`/cookies. HTML over 4 MB falls back to the redirect (serverless response caps). The studio preview of a draft HTML file wraps the fetched bytes in a sandboxed `srcdoc` iframe (`sandboxedHtmlUrl` in `lib/library.js`) since a fresh tab has no bearer token. This is also why a resource's public detail page validates that the URL's username segment is actually one of its credited mentors before rendering — a guessed or stale link should not resolve.
