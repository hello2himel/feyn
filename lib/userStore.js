...
import { getSupabase, getSupabaseReady } from './supabase'
...
export async function enroll(programId, subjectId) {
  ...
  if (!list.includes(key)) {
    ...
    if (isGlobalAccount()) {
      const sb = await getSupabaseReady(); const p = getProfile()
      if (sb && p?.supabaseId)
...
export async function markWatched(programId, subjectId, topicId, lessonId) {
  ...
  if (isGlobalAccount()) {
    const sb = await getSupabaseReady(); const prof = getProfile()
...
export async function issueCert(programId, subjectId, subjectName, programName, userName) {
  ...
  if (!isGlobalAccount()) return { cert, dbOk: false, dbError: null }  // local account, no DB
  ...
  // getUser() makes a real network call to verify the token and always returns
  // the current user — unlike getSession() which can return null on first load
  // if the session hasn't been hydrated from localStorage yet.
  const { data: userData, error: userError } = await sb.auth.getUser()
  if (userError || !userData?.user) {
    console.error('[Feyn] issueCert: no active user', userError)
