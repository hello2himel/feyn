...
export default function VerifyPage() {
  ...
  const certId = query.certId

  const [state,  setState]  = useState('loading')
  const [cert,   setCert]   = useState(null)

  useEffect(() => {
    if (!isReady || !certId) return
      ...
      <Head>
        <title>
          {state === 'valid' || state === 'local'
            ? `Certificate · ${cert?.subject_name || cert?.subjectName} — Feyn`
                ...
                <div className="verify-details">
                  ...
                  <div className="verify-details__row">
                    ...
                    <span className="verify-details__value">
                      {cert.subject_name || cert.subjectName}
                    </span>
                  </div>
                  <div className="verify-details__row">
                    <span className="verify-details__label">Program</span>
                    <span className="verify-details__value">
                      {cert.program_name || cert.programName}
                    </span>
                  </div>
                  {dateStr && (
                    <div className="verify-details__row">
            ...
            {state === 'invalid' && (
              <div className="verify-card verify-card--invalid">
                ...
                <p className="verify-card__sub">
                  ...
                  in Feyn's records. It may have been issued to a local account, revoked,
