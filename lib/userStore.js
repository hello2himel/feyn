...
export async function issueCert(programId, subjectId, subjectName, programName, userName) {
  ...
  // Use existing cert if found, otherwise create a new one
  const cert = existing || (() => {
    const c = {
  ...
  // For global accounts: always upsert to Supabase.
  // This covers three cases:
  //   1. Brand new cert — insert for the first time
  //   2. Re-download — cert in localStorage but was never written to DB
  //      (e.g. insert failed silently before this fix, or require() bug)
  //   3. Cert already in DB — upsert is a no-op, harmless
  if (isGlobalAccount()) {
    const sb = getSupabase()
    if (!sb) {
    ...
    // Explicitly get the session — forces Supabase v2 to restore it from
    // localStorage if the client was just initialised on this page load.
    const { data: sessionData, error: sessionError } = await sb.auth.getSession()
    if (sessionError || !sessionData?.session) {
      console.error('[Feyn] issueCert: no active session', sessionError)
      return cert
    }
